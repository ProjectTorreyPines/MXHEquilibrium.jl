...
Keyword Arguments:\\
'Z0`- Elevation [m]\\`B0_dir`- Direction of the toroidal field (+1 = CCW, -1 = CW)\\`Ip_dir`- Direction of the plasma current (+1 = CCW, -1 = CW)\\`diverted`- If true then equilibrium has one or more x-points\\`x_point`- If diverted = true, then x_point is set to (R0*(1-1.1*δ*ϵ), -R0*1.1*κ*ϵ) else nothing\\`symmetric` - Is equilibrium up-down symmetric (must be if not diverted)\\
"""
function solovev(B0, R0, ϵ, δ, κ, α, qstar; Z0=zero(R0), ζ=zero(δ), kwargs...)
    return solovev(B0, TurnbullMillerShape(R0, Z0, ϵ, κ, δ, ζ), α, qstar; kwargs...)
end
...
function Base.show(io::IO, S::SolovevEquilibrium)
    ...
    return print(io, "  symmetric = $(S.symmetric)")
end
...
function limits(S::SolovevEquilibrium; pad=0.2)
    return xlims, ylims = limits(shape(S), S.x_point; pad=pad)
end
