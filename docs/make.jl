using Documenter, MXHEquilibrium

# Call functions
open(joinpath(@__DIR__, "src/api.md"), "w") do f
    println(f, "# API Reference\n")
    for page in keys(MXHEquilibrium.document)
        if page == :Expressions
            continue
        end
        println(f, "## $page\n")
        println(f, "```@docs")
        for item in MXHEquilibrium.document[page]
            println(f, "$item")
        end
        println(f, "```")
    end
...
makedocs(;
    modules=[MXHEquilibrium],
    format=Documenter.HTML(;analytics="G-65D8V8C8VQ"),
    sitename="MXHEquilibrium",
    checkdocs=:none,
...
# https://projecttorreypines.github.io/MXHEquilibrium.jl/stable
# will be built from latest release tagged with a version number.
# The development documentation that will be hosted on
# https://projecttorreypines.github.io/MXHEquilibrium.jl/dev
...
deploydocs(;
    repo="github.com/ProjectTorreyPines/MXHEquilibrium.jl.git",
    target="build",
    branch="gh-pages",
    devbranch="master",
    versions=["stable" => "v^", "v#.#"]
